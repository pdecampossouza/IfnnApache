...
This script:
  ...
  * Runs 10 repeated experiment runs with different random seeds:
      - 70% training / 30% testing split (stratified) for each run.
      - Standardises the features (train-set statistics) and uses the
        **same normalised data for all models**, including the FNN.
...
# Optional: silence most TensorFlow C++ logs if TF is installed in the environment
...
FNN_ACTIVATION = "linear"
FNN_OPTIMIZER = "adam"  # moore-penrose, adam, pso
FNN_PRUNING = "none"
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    See README_Script2_normalized.md, Section 4 for the formal definitions.
    """
...
def train_and_evaluate_baselines(
    ...
    """Train traditional classifiers on normalised features and evaluate them.

    Parameters
    ----------
    X_train, X_test : np.ndarray
        Already-standardised design matrices (same scaling as used for FNN).
    y_train, y_test : np.ndarray
        Binary labels.
    seed : int
        Random seed for reproducible initialisations.
    """
...
def train_and_evaluate_fnn(
    ...
    The inputs X_train and X_test are the *normalised* features, so the
    FNN is trained in the same scaled space as the baselines.
    """
    ...
    # Use a numpy Generator, as expected by your original FNN implementation
    ...
    fnn_model.train_model(X_train, y_train_signed)
    y_pred_signed, _ = fnn_model.evaluate_model(X_test, y_test_signed)
...
def save_fnn_rules(fnn_model, output_dir: Path, max_rules_for_tex: int = 15) -> None:
    ...
    with tex_path.open("w", encoding="utf-8") as f:
        ...
        f.write(
            "\caption{Example of fuzzy rules induced by the FNN model (Apache due-date dataset, seed 0).}"
            "\label{tab:fnn-rules-apache}\n"
        )
...
def main() -> None:
    logger.info(
        "Starting Script 2: FNN vs. baseline classifiers (all using normalised features)."
    )
    ...
    runs_records: List[Dict] = []  # will collect metrics for each (model, seed)
    ...
    for seed in tqdm(RANDOM_SEEDS, desc="Runs"):
        ...
        # ------------------------------------------------------------------
        # Standardisation: SAME SCALER for FNN and all baselines
        # ------------------------------------------------------------------
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train_raw)
        X_test = scaler.transform(X_test_raw)

        # FNN (uses normalised features)
        ...
        # Baselines (use the same normalised features)
